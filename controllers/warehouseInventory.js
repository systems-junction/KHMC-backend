...
exports.getWhInventory = (req, res, next) => {
  try {
    ...
    res.status(400).send({
      success: false,
      message: 'Error getting warehouse inventory!',
      error: e.toString(),
    });
  }
};
    ...
    if (
      ...
      (ware[i].itemId.name &&
        ware[i].itemId.name
          .toLowerCase()
          .startsWith(req.params.keyword.toLowerCase())) ||
      (ware[i].itemId.tradeName &&
        ware[i].itemId.tradeName
          .toLowerCase()
          .startsWith(req.params.keyword.toLowerCase())) ||
      (ware[i].itemId.scientificName &&
        ware[i].itemId.scientificName.startsWith(req.params.keyword))
    ) {
      arr.push(ware[i]);
    }
  }
...
exports.addWhInventory = (req, res, next) => {
  ...
  try {
    WhInventory.create({
      ...
      (async () => {
        try {
          const data = await fetch(blockchainUrl + 'addWarehouseInventory', {
            method: 'POST',
            headers: {
      ...
      res.status(200).send({
        success: true,
        message: 'Warehouse inventory added successfully',
      });
    });
  } catch (e) {
    res.status(400).send({
      success: false,
      message: 'Error adding warehouse inventory!',
      error: e.toString(),
    });
  }
};
...
exports.deleteWhInventory = (req, res, next) => {
  ...
  try {
    ...
    res.status(400).send({
      success: false,
      message: 'Error deleting warehouse inventory!',
      error: e.toString(),
    });
  }
};
...
exports.updateWhInventory = (req, res, next) => {
  ...
  try {
    WhInventory.updateOne({ _id: _id }, req.body).then(function () {
      res.status(200).send({
        success: true,
        message: 'Warehouse inventory update successfully',
      });
    });
  } catch (e) {
    res.status(400).send({
      success: false,
      message: 'Error updating warehouse inventory!',
      error: e.toString(),
    });
  }
};
exports.getExpiredInventory = asyncHandler(async (req, res) => {
  var todayDate = moment().startOf('day').utc().toDate();
