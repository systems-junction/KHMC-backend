...
exports.getClaimsKeyword = asyncHandler(async (req, res) => {
  const rc = await RC.find()
    ...
    {
       if(
      (rc[i].requestNo && rc[i].requestNo.toLowerCase().startsWith(req.params.keyword.toLowerCase())))
      {
        arr.push(rc[i])
      }
    }
...
exports.getPatient = asyncHandler(async (req, res) => {
  var array=[]
  var secondArray=[]
  const ipr = await IPR.find({functionalUnit:req.params.id, status: { $ne: "Discharged" }}).populate('patientId','profileNo firstName lastName SIN mobileNumber phoneNumber age gender drugAllergy weight').select({patientId:1})
    for(let i = 0; i<ipr.length; i++)
    {
        array.push(ipr[i].patientId) 
    }
  const edr = await EDR.find({ status: { $ne: "Discharged" }}).populate('patientId','profileNo firstName lastName SIN mobileNumber phoneNumber age gender drugAllergy weight').select({patientId:1})
