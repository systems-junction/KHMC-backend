...
exports.getClaims = asyncHandler(async (req, res) => {
    const rc = await RC.find().populate('generatedBy').populate('patient').populate('insurer')
    res.status(200).json({ success: true, data: rc });
});

exports.getPatient = asyncHandler(async (req, res) => {
    const patient = await Patient.find({
    $or: [{ profileNo: {$regex: req.params.keyword, $options: 'i'}},{ firstName: {$regex: req.params.keyword, $options: 'i'}},{ lastName: {$regex: req.params.keyword, $options: 'i'}},
...
exports.getEDRorIPR = asyncHandler(async (req, res) => {
    ...
    var dateEdr
    var dateIpr
    const edr = await EDR.findOne({patientId:req.params._id},{},{sort:{'createdAt':-1}})
    ...
    const ipr = await IPR.findOne({patientId:req.params._id},{},{sort:{'createdAt':-1}})
    .populate('pharmacyRequest.medicine.itemId')
    .populate('labRequest.serviceId')
    .populate('radiologyRequest.serviceId')
...
exports.updateClaims = asyncHandler(async (req, res, next) => {
    ...
    if (!rc) {
      return next(new ErrorResponse(`Reimbursement Claim not found with id of ${_id}`, 404));
    }
    if (req.file) {
