...
exports.addSurgeryService = asyncHandler(async (req, res) => {
  ...
  const surgeryService = await SurgeryService.create({
    serviceNo: 'SS' + requestNoFormat(new Date(), 'mmddyyHHmm'),
    name,
    description,
    price,
    status,
  });
  notification("Surgery Service", "A new surgery service has been added", "admin")
...
exports.updateSurgeryService = asyncHandler(async (req, res, next) => {
  ...
  surgeryService = await SurgeryService.findOneAndUpdate(
    { _id: _id },
    req.body,
    { new: true }
  );
