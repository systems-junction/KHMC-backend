...
exports.getInternalReturnRequestsFU = asyncHandler(async (req, res) => {
  const internalRequestFU = await InternalReturnRequest.find({
    ...
    .populate('fuId')
    .populate('itemId')
    .populate('replenishmentRequestFU');
  res.status(200).json({ success: true, data: internalRequestFU });
});

exports.getInternalReturnRequestsFUByKeyword = asyncHandler(
  async (req, res) => {
    const internalRequestFU = await InternalReturnRequest.find({
      to: 'Warehouse',
      from: 'FU',
...
exports.getInternalReturnRequestsBU = asyncHandler(async (req, res) => {
  const internalRequestBU = await InternalReturnRequest.find({
    to: 'FU',
    from: 'BU',
  })
    .populate('buId')
...
exports.addInternalReturnRequest = asyncHandler(async (req, res) => {
  ...
  const send = await InternalReturnRequest.find({ to: 'Warehouse', from: 'FU' })
    .populate('fuId')
    .populate('itemId')
    .populate('replenishmentRequestFU');
...
exports.updateInternalRequest = asyncHandler(async (req, res, next) => {
  ...
  if (req.body.status == 'approved') {
    ...
    const send = await InternalReturnRequest.find({
      to: 'Warehouse',
      from: 'FU',
    })
      .populate('fuId')
    ...
    if (req.body.to == 'Warehouse' && req.body.from == 'FU') {
      const receive = await receiveItemFU.findOne({
        replenishmentRequestId: req.body.replenishmentRequestFU,
      });
      ...
      for (let i = 0; i < internalRequest.returnBatchArray.length; i++) {
        for (let j = 0; j < fuWithUpdatedQty.batchArray.length; j++) {
          if (
            fuWithUpdatedQty.batchArray[j].batchNumber ===
            internalRequest.returnBatchArray[i].batchNumber
          ) {
            newBatchArray[j] = {
      ...
      await ReplenishmentRequest.findOneAndUpdate(
        {
          _id: req.body.replenishmentRequestFU,
          'items.itemId': req.body.itemId,
        },
        {
          $set: {
            'items.$.secondStatus': 'Returned',
  ...
  internalReturn = await InternalReturnRequest.findOneAndUpdate(
    { _id: _id },
    req.body,
    { new: true }
  );
